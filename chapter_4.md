...
Reward-modulated Spike-Timing-Dependent Plasticity (R-STDP) is the learning rule this hardware was extended to support. It adds biologically inspired synaptic plasticity gated by a delayed reward signal — the substrate for reinforcement learning on a spiking neural network.

Chapter 4 is split into three arcs:

- **4.1–4.2** — the *concept* and the existing host-side machinery (`hs_api` synapse R/W).
- **4.3–4.9** — the *new hardware* that runs R-STDP without host involvement on the hot path. This is where most of Diana's recent work lives.
- **4.10–4.12** — *verification and next steps* — how to test changes in simulation, how to drive the system through commands, and where new students should pick up.

If you're new to the project, read [4.3](4_Implementing_RSTDP/4_3_rstdp_hardware_roadmap) first — it's the navigation hub for the hardware arc and tells you what assumptions each page makes.
...
| 4.5 | [Tracking R3 Addresses Through HBM Bursts](4_Implementing_RSTDP/4_5_r3_address_tracking) | The burst-descriptor FIFO inside `hbm_processor.v` — how each 512-bit synapse packet gets tagged with its R3 address. |
